...
public class ChatWithImageView extends VerticalLayout {
	...
	public ChatWithImageView(ChatService chatService) {
		...
		resetButton.addClickListener(event -> {
			...
			MessageListItem welcome = new MessageListItem("Hello there! Upload images to start chatting with AI.",
					Instant.now(), "AI");
			welcome.setUserAbbreviation("AI");
			welcome.setUserColorIndex(2);
			chat.setItems(welcome);
	...
	private void onSubmit(MessageInput.SubmitEvent submitEvent) {
		...
		Thread t = new Thread(() -> chatService.askWithImages(submitEvent.getValue(), imageFiles, modelSelected,
